...
function DashboardContent() {
  ...
  const renderActiveView = () => {
    ...
    const counts = {
      draft: currentItems.filter(i => i.status === '작성중').length,
      review: currentItems.filter(i => i.status === '검토필요' || i.status === '소명중').length,
      complete: currentItems.filter(i => i.status === '정상').length
    };
    ...
    switch (activeView) {
        ...
        return (
          <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar bg-[#f5f5f7]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: '작성 중', count: counts.draft, key: '작성중', description: '연구자 입력 대기' },
                { label: '검토 중', count: counts.review, key: '검토필요', description: 'AI 이상징후 탐지' },
                { label: '정산 완료', count: counts.complete, key: '정상', description: '최종 승인 완료' }
              ].map((stat, i) => (
                <div 
                  key={i} 
                  onClick={() => setStatusFilter(statusFilter === stat.key ? "all" : stat.key)}
                  className={cn(
                    "bg-white p-8 rounded-[24px] border border-[#d2d2d7] shadow-sm relative overflow-hidden group hover:border-[#86868b] transition-all cursor-pointer active:scale-95",
      ...
      case "evidence":
        return <EvidenceRecordView items={currentItems} />;
      case "stats":
        return (
          <div className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar bg-[#1d1d1f]">
            ...
            <div className="min-h-[60vh] rounded-[32px] bg-[#272729] flex flex-col items-center justify-center text-center p-20 shadow-2xl">
              ...
              <p className="text-[#86868b] text-[17px] max-w-md leading-relaxed">
                과거 집행 패턴과 현재 데이터 스트림을 실시간으로 비교 분석 중입니다.
              </p>
            </div>
          </div>
        );
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  ...
                  <Button 
                    ...
                    className="h-9 text-[14px] font-medium text-[#0066cc] hover:bg-[#e3f2fd] rounded-full px-4"
                  >
